...
async function main() {
  ...
  try {
    ...
    await session.run(`CREATE CONSTRAINT studio_name IF NOT EXISTS FOR (s:Studio) REQUIRE s.name IS UNIQUE`);
    ...
    await session.run(`MATCH (n) WHERE n:Movie OR n:Person OR n:Genre OR n:Studio OR n:User DETACH DELETE n`);
    ...
    for (const movie of movies) {
      ...
      await session.run(
        `
        MERGE (m:Movie {id: $id})
        SET m.title = $title, m.year = $year, m.runtime = $runtime, m.plot = $plot
